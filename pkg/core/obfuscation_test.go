...
func TestIsValidActionRef(t *testing.T) {
	tests := []struct {
		name      string
		usesValue string
		want      bool
		...
		{
			name:      "local action with ../",
			usesValue: "../my-action",
			want:      true,
		},
		{
			name:      "commit sha reference",
			usesValue: "actions/checkout@a5ac7e51b41094c92402da3b24376905380afc29",
			want:      true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := isValidActionRef(tt.usesValue)
			if result != tt.want {
				t.Errorf("isValidActionRef(%q) = %v, want %v", tt.usesValue, result, tt.want)
			}
		})
	}
}
